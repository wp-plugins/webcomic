...
class webcomic extends mgs_core {
	...
	public function get_relative_webcomics( $taxonomy = false, $terms = false, $id = false, $global = false ) {
		...
		$query = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM $wpdb->posts AS p $join WHERE p.post_type = 'webcomic_post' AND p.post_status IN ( 'publish', 'private' ) ORDER BY p.post_date ASC", false ) );
		$r     = array();
